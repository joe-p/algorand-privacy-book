...
Mithras started as a fork of the [Hermes Vault](./hermes.md) mixer with two additional requirements:

- Users should be able to spend funds within the protocol without revealing accounts or amounts
...
- To keep addresses private, we need senders to be able to derive new public keys that receivers own but otherwise appear random
  - We can't reuse the same address every time otherwise transactions can be linked
  - [HD xPub](../primitives/hd.md) is not viable because it would allow all spenders sending to the same address see each other's transactions
    - Would also require off-chain sharing of xPub
  - Solution: [stealth addresses](../primitives/stealth.md) used for *spend key* (Ed25519)
